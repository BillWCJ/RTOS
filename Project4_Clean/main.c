...
unsigned short getColour(void){
	return (rand(sizeof(colourList)-1))
}
volatile colourList[] = {Black, Navy, DarkGreen, DarkCyan, Maroon, Purple, Olive, LightGrey, DarkGrey, Blue, Green, Cyan, Red, Magenta, Yellow}
...
// 	turnOnLED(balls-1);
	os_tsk_create_ex(user_task, 1, (void*)ballParam[balls-1]);
	os_tsk_prio_self ( 1 );
...
int main( void ) {
  // system initalization
	...
	for( i = 0; i < 8; i++){
		...
		ballParam[i]->size_x = 10;
		ballParam[i]->size_y = 10;
	}
